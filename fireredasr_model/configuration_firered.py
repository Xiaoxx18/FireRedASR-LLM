...
"""FireRedASR_LLM model configuration"""
...
class FireredAudioEncoderConfig(PretrainedConfig):
    r"""
    This is the configuration class to store the configuration of a [`FireredAudioConfig`]. It is used to instantiate a
    FireredAudio audio encoder according to the specified arguments, defining the model architecture. Instantiating a
    configuration with the defaults will yield a similar configuration to that of the audio encoder of the FireredAudio
    architecture.
    ...
    ```python
    >>> from transformers import FireredAudioEncoderConfig, Qwen2AudioEncoder

    >>> # Initializing a FireredAudioEncoderConfig
    >>> configuration = FireredAudioEncoderConfig()
...
class FireredAudioConfig(PretrainedConfig):
    ...
    ```python
    >>> from transformers import Qwen2AudioForConditionalGeneration, FireredAudioConfig, FireredAudioEncoderConfig, Qwen2Config
    ...
    >>> configuration = FireredAudioConfig(audio_config, text_config)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(audio_config, dict):
            audio_config["model_type"] = audio_config.get("model_type", "firered_audio_encoder")
            audio_config = CONFIG_MAPPING[audio_config["model_type"]](**audio_config)
        elif audio_config is None:
            audio_config = CONFIG_MAPPING["firered_audio_encoder"](
                d_model=1280,
                encoder_attention_heads=20,
                encoder_ffn_dim=5120,
                encoder_layerdrop=0.0,
...
__all__ = ["FireredAudioConfig", "FireredAudioEncoderConfig"]
